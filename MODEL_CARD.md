...
- **Training / tuning (shipped artefacts):** `run_training.py`, fixed hyperparameters, best-of-candidates selection on the **val** split, with **test** scored exactly once by the already-selected model. Full record with provenance (commit SHA, sklearn version, seed, splits) in `reports/training_metrics.json`.
  - **Provenance-SHA caveat:** neither recorded `commit_sha` is reachable from `main`. This repo squash-merges, which orphans branch commits, so both `benchmarks/results.json` and `reports/training_metrics.json` name PR-branch commits that are **not ancestors of `main`**. The training run at least started from a clean tree (`working_tree_clean: true`), so its artefacts match that commit's source exactly. What pins the artefacts instead is `models/MANIFEST.sha256`, which records a SHA-256 for each of the four governed files; `tests/test_artifact_manifest.py` recomputes them and fails on any drift. That ties the shipped bytes to the committed manifest, not to the source.
